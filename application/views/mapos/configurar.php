  ...
  <div class="span12">
    <div class="widget-box">
      ...
      <ul class="nav nav-tabs">
        ...
        <li><a data-toggle="tab" name="btn-menu-financeiro" href="#menu-financeiro">Financeiro</a></li>
        <li><a data-toggle="tab" name="btn-menu-produtos" href="#menu-produtos">Produtos</a></li>
        <li><a data-toggle="tab" name="btn-menu-notificacoes" href="#menu-notificacoes">Notificações</a></li>
        <li><a data-toggle="tab" name="btn-menu-os" href="#menu-os">OS</a></li>
      </ul>
      <form action="<?php echo current_url(); ?>" id="formConfigurar" method="post" class="form-horizontal">
        <div class="widget-content nopadding tab-content">
          ...
          <!-- Menu Notificações -->
          <div id="menu-notificacoes" class="tab-pane fade">
            <div class="control-group">
          ...
          <!-- Menu Financeiro -->
          <div id="menu-financeiro" class="tab-pane fade">
            <div class="control-group">
          ...
          <!-- Menu Produtos -->
          <div id="menu-produtos" class="tab-pane fade">
            <div class="control-group">
          ...
          <!-- Menu Atualização
          <div id="menu-atualizacoes" class="tab-pane fade">
            <div class="form-actions">
              <div class="span8">
          ...
          <!-- Menu OS -->
          <div id="menu-os" class="tab-pane fade">
            <div class="control-group">
              ...
              <div class="span8" style="margin-left: 3em;">
                <label for="checklist_os" class="control-label">Checklist na abertura da OS</label>
                <div class="controls">
                  <select name="checklist_os" id="checklist_os">
                    <option value="1">Ativar</option>
                    <option value="0" <?= $configuration['checklist_os'] == '0' ? 'selected' : ''; ?>>Desativar</option>
                  </select>
                  <span class="help-inline">Ativar ou desativar a utilização de checklists na abertura da OS.</span>
                </div>
              </div>
              <div class="span8 hide" name="container-checklists">
                <div class="widget-title">
                  <span class="icon">
                    <i class="fas fa-wrench"></i>
                  </span>
                  <h5 style="padding-left:0px;">Checklists</h5>
                </div>
                <?php
                  printf('<a href="%schecklist/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px;">', base_url());
                ?>
                <span class="button__icon"><i class="bx bx-plus-circle"></i></span><span class="button__text2"> Checklists</span></a>
                <div class="widget-box">
                  <div class="widget-content nopadding tab-content">
                    <table class="table table-bordered ">
                      ...
                      <tbody>
                        <?php
                          $oResult = $this->mapos_model->getChecklists();
                          if (!$oResult) {
                            printf('<tr><td colspan="5">Nenhum Checklist Cadastrado</td></tr>');
                          }
                          $sTagImpressao = '
                            <tr>
                              <td>%s</td>
                              <td style="width: 50px;">
                                <a href="#modal-excluir-checklist" name="btn-excluir-checklist" role="button" data-toggle="modal" data-checklist="%d" class="btn-nwe4" title="Excluir Checklist"><i class="bx bx-trash-alt bx-xs"></i></a>
                              </td>
                            </tr>
                          ';
                          foreach ($oResult as $oLinha) {
                            printf(
                                $sTagImpressao
                              , $oLinha->nome
                              , $oLinha->idChecklist
...
<!-- Modal excluir checklist -->
<div id="modal-excluir-checklist" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h5 id="myModalLabel">Excluir Checklist</h5>
  </div>
  <div class="modal-body">
    <input type="hidden" id="idChecklist" name="id" value="" />
    <h5 style="text-align: center">Deseja realmente excluir este checklist?</h5>
  </div>
  <div class="modal-footer" style="display:flex;justify-content: center">
    <button class="button btn btn-warning" data-dismiss="modal" aria-hidden="true"><span class="button__icon"><i class="bx bx-x"></i></span><span class="button__text2">Cancelar</span></button>
    <button class="button btn btn-danger" name="btn-confirm-excluir-checklist">
...
<script>
  $(document).ready(function() {
    $('[name=btn-confirm-excluir-checklist]').on('click', function(event) {
      $.ajax({
          type: 'POST'
        , url: '<?=base_url();?>checklist/excluir'
        , data: {
          'id' : $('#idChecklist').val()
        }
        , dataType: 'json'
        , success: function(data) {
          if (data.status == 'ok') {
            jLocation('<?=base_url()?>mapos/configurar?menu=os');
          } else {
            Swal.fire({
                type: "error",
                title: "Atenção",
                text: "Ocorreu um erro ao excluir o checklist."
            });
          }
        }
      });
    });
    $('[name=btn-excluir-checklist]').on('click', function(event) {
      $('#idChecklist').val($(this).data('checklist'));
    });
    <?php
      if(!empty($_GET['menu'])) {
        printf('$("[name=btn-menu-%s]").tab("show");', $_GET['menu']);
      }
    ?>
    $('[name=checklist_os]').change(function() {
      jShowHideChecklist();
    });
  ...
  function jShowHideChecklist() {
    $('[name=container-checklists]').hide();
    if($('[name=checklist_os]').val() == '1') {
      $('[name=container-checklists]').show();
    }
  }
</script>
